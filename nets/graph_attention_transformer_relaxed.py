...
from .tensor_product_rescale import (TensorProductRescale,
...
from .relaxed_linear import RelaxedLinearRS
...
class SeparableFCTP(torch.nn.Module):
    ...
    def __init__(self, irreps_node_input, irreps_edge_attr, irreps_node_output, 
        ...
        self.lin = RelaxedLinearRS(self.dtp.irreps_out.simplify(), irreps_lin_output)
...
class GraphAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # Merge src and dst
        self.merge_src = RelaxedLinearRS(self.irreps_node_input, self.irreps_pre_attn, bias=True)
        self.merge_dst = RelaxedLinearRS(self.irreps_node_input, self.irreps_pre_attn, bias=False)
        ...
        if self.nonlinear_message:
            ...
            self.sep_alpha = RelaxedLinearRS(self.sep_act.dtp.irreps_out, irreps_alpha)
        ...
        self.proj = RelaxedLinearRS(irreps_attn_heads, self.irreps_node_output)
        self.proj_drop = None
        if proj_drop != 0.0:
...
class NodeEmbeddingNetwork(torch.nn.Module):
    ...
    def __init__(self, irreps_node_embedding, max_atom_type=_MAX_ATOM_TYPE, bias=True):
        ...
        self.atom_type_lin = RelaxedLinearRS(o3.Irreps('{}x0e'.format(self.max_atom_type)), 
            self.irreps_node_embedding, bias=bias)
        # TODO: find out why we do this and change accordingly
...
class EdgeDegreeEmbeddingNetwork(torch.nn.Module):
    def __init__(self, irreps_node_embedding, irreps_edge_attr, fc_neurons, avg_aggregate_num):
        super().__init__()
        self.exp = RelaxedLinearRS(o3.Irreps('1x0e'), irreps_node_embedding, 
        ...
        self.proj = RelaxedLinearRS(self.dw.irreps_out.simplify(), irreps_node_embedding)
...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        self.head = torch.nn.Sequential(
            RelaxedLinearRS(self.irreps_feature, self.irreps_feature, rescale=_RESCALE), 
            Activation(self.irreps_feature, acts=[torch.nn.SiLU()]),
            RelaxedLinearRS(self.irreps_feature, o3.Irreps('1x0e'), rescale=_RESCALE)) 
...
@register_model
def graph_attention_transformer_l2_relaxed(irreps_in, radius, num_basis=128, 
...
@register_model
def graph_attention_transformer_nonlinear_l2_relaxed(irreps_in, radius, num_basis=128, 
    atomref=None, task_mean=None, task_std=None, **kwargs):
    model = GraphAttentionTransformer(
        irreps_in=irreps_in,
        irreps_node_embedding='128x0e+64x1e+32x2e', num_layers=6,
...
@register_model
def graph_attention_transformer_nonlinear_l2_e3_relaxed(irreps_in, radius, num_basis=128, 
    atomref=None, task_mean=None, task_std=None, **kwargs):
    model = GraphAttentionTransformer(
        irreps_in=irreps_in,
...
@register_model
def graph_attention_transformer_nonlinear_bessel_l2_relaxed(irreps_in, radius, num_basis=128, 
    atomref=None, task_mean=None, task_std=None, **kwargs):
    model = GraphAttentionTransformer(
        irreps_in=irreps_in,
        irreps_node_embedding='128x0e+64x1e+32x2e', num_layers=6,
...
@register_model
def graph_attention_transformer_nonlinear_bessel_l2_drop01_relaxed(irreps_in, radius, num_basis=128, 
    atomref=None, task_mean=None, task_std=None, **kwargs):
    model = GraphAttentionTransformer(
        irreps_in=irreps_in,
        irreps_node_embedding='128x0e+64x1e+32x2e', num_layers=6,
...
@register_model
def graph_attention_transformer_nonlinear_bessel_l2_drop00_relaxed(irreps_in, radius, num_basis=128, 
    atomref=None, task_mean=None, task_std=None, **kwargs):
    model = GraphAttentionTransformer(
        irreps_in=irreps_in,
        irreps_node_embedding='128x0e+64x1e+32x2e', num_layers=6,
